...
async function spawnAsync(command, args, options, liveOutput = false) {
...
async function runInExamples(command, args, examples, printOutput = false) {
    ...
    const rejected = promiseResults.filter(r => r.status === 'rejected');
    const fulfilled = promiseResults.filter(r => r.status === 'fulfilled');
...
async function updateExampleDependencies(examplesToUpdate) {
...
function exit(exitCode, messageOrError) {
    if (exitCode > 0) {
